...
import SaveToProjectModal from '../components/SaveToProjectModal.jsx';
...
export default function SubmittalExport() {
  ...
  function handleDownload() {
    ...
    a.href = getReportUrl(id);
    a.download = '';
    a.click();
  }

  const CTA = <button className="btn-primary" onClick={() => setShowSaveModal(true)}>Stamp & submit →</button>;
  ...
  return (
    <>
      {showSaveModal && (
        <SaveToProjectModal
          analysisId={id}
          analysisName={analysis?.project_name || 'Analysis'}
          onSave={() => { setShowSaveModal(false); handleDownload(); }}
          onSkip={() => { setShowSaveModal(false); handleDownload(); }}
